...
int av_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;){
        ...
        if (pktl) {
            *pkt = pktl->pkt;
            if(s->streams[pkt->stream_index]->codec->codec_id != CODEC_ID_PROBE){
                ...
                av_free(pktl);
				//av_log(NULL, AV_LOG_DEBUG, "%s: alloc_count:%d., codec is not ID_PROBE\n", 
		...
		if(alloc_count > 500)/* I guess it is about 43.6K */
		{/* For one packet, we try to get too much data from media file, so return. */
			av_log(NULL, AV_LOG_DEBUG, "%s: alloc_count:%d for a packet, over 100. \n", 
										__FUNCTION__, alloc_count);
			return -1;
		}
		//alloc_count ++;
        ...
        if(!pktl && st->codec->codec_id!=CODEC_ID_PROBE){
			//av_log(NULL, AV_LOG_DEBUG, "%s: alloc_count:%d., got CODEC_ID:%d.\n \n", 
			//							__FUNCTION__, 
			//							alloc_count, 
			//							st->codec->codec_id);
            return ret;
		...
		if(mem_alloc_size > 52428) /*about 50K*/
		{
#ifdef OOM_DEBUG
			av_log(NULL, AV_LOG_DEBUG, "%s: alloc_count:%d, size:%d  for a packet. \n",
										__FUNCTION__, 
										alloc_count,
										mem_alloc_size);
#endif
        ...
        if(st->codec->codec_id == CODEC_ID_PROBE){
			...
			alloc_count ++;	
			mem_alloc_size = pd->buf_size;	
			
            memcpy(pd->buf+pd->buf_size, pkt->data, pkt->size);
            pd->buf_size += pkt->size;

#ifdef OOM_DEBUG			
			av_log(NULL, AV_LOG_DEBUG, "%s:pd->buf_size:%d\n", __FUNCTION__, pd->buf_size);
#endif
            memset(pd->buf+pd->buf_size, 0, AVPROBE_PADDING_SIZE);
...
static int av_read_frame_internal(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for(;;) {
        ...
        if (st) {
			...
			//av_log(NULL, AV_LOG_DEBUG, " st exists.\n");
...
#if 1
			/* []FIXME] Tring to get mpore data for geting stream info, but
 				We need to stop it? */
			if(count >=300) /*50k*100 = 5M*/
			{/* For decoding, to get too much packets. */
#ifdef OOM_DEBUG
				av_log(NULL, AV_LOG_DEBUG, "Read too much packet:%d. give up decoding. \n", count);
#endif
				ret = AVERROR(EAGAIN);
				return ret;
			}
...
#if  0
		//av_log(NULL, AV_LOG_DEBUG, "-> %d. %s. v.100\n", count, __FUNCTION__);
		if(count >=100 )
		{
			//av_log(NULL, AV_LOG_DEBUG, "Try too much times, return error.\n");
			//return AVERROR(ENOMEM);
		}
#endif
...
//                    av_log(NULL, AV_LOG_ERROR, "%f\n", dur);
...
#if 0			
			if(try_count >=5 ){
				av_log(NULL, AV_LOG_DEBUG, "-> give up trying decode frame, because tried %d times.\n",
						try_count);
				//return AVERROR(ENOMEM);
				break;
			}
#endif
#ifdef OOM_DEBUG
			av_log(NULL, AV_LOG_DEBUG, "->No codec information, we try to decode frame,"
										"pkt->size:%d, st->codec_id:%d.\n",
										pkt->size, st->codec->codec_id);
#endif
			try_count ++;
        ...
        //count++; /* For how much packet being read. */
...
#ifdef OOM_DEBGU
	av_log(NULL, AV_LOG_DEBUG, "===> close the codec.\n\n");
#endif
