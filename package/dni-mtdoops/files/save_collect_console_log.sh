...
mtd_oops="$(find_mtd_part 'mtdoops')"
...
echo close > /sys/devices/platform/serial8250.0/console
...
if [ "x$collect_log" = "x1" ];then
	zip debug-log.zip Panic-log.txt Console-log1.txt Console-log2.txt basic_debug_log.txt lan.pcap wan.pcap
else
	zip debug-log.zip NETGEAR_$module_name.cfg Panic-log.txt Console-log1.txt Console-log2.txt basic_debug_log.txt lan.pcap wan.pcap
fi

cd /tmp
rm -rf debug-usb debug_cpu debug_flash debug_mem debug_mirror_on debug_session NETGEAR_$module_name.cfg Panic-log.txt Console-log1.txt Console-log2.txt basic_debug_log.txt lan.pcap wan.pcap
