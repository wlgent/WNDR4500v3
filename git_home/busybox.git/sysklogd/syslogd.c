...
//As netgear  weber requirement, cut the access control log follow that:
//orgin:[Access Control] Device MAXIMEGALON  with MAC address 84:3A:1B:48:5D:8A is allow to access the network, Tuesday, September 16, 2014 18:32:35
//cut:  [Access Control] Device <Device Name>[<Mac address] is <allowed or blocked> to access the network

void cut_acl_log(char *msg, char *cut_str, int max_log_len)
{
	char *cut_index, *is_index;
	char *cut_end;
	int cut_len, msg_len;


	cut_index = strstr(msg, cut_str);
	is_index = strstr(msg, " is ");
	if(cut_index == NULL || is_index == NULL)
		return;

	cut_len = strlen(cut_str);
	cut_end = cut_index + cut_len;

	*is_index = '\0';
	++is_index;
	
	/* 84:3A:1B:48:5D:8A change to [84:3A:1B:48:5D:8A] */
	*cut_index = '\0';
	*cut_end = '[';
	/* use pointer operate program stack, not need to define a tmp array tmp[1024] for strcat function. */
	strcat(msg, cut_end);
	strcat(msg, "] ");
	strcat(msg, is_index);
	...
	msg_len = strlen(msg);
	/* the length for msg still more than max_log_len? continue to cut,and the string "the network" will be cut */
	if(msg_len >  max_log_len){
		char *tmpstr = " the network";
		cut_index = strstr(msg, tmpstr);
		*cut_index = '\0';

		cut_len = strlen(tmpstr);
		cut_end = cut_index + cut_len;
 ...
 * To fix bug 45131:[Access Control] Device unknown with......is allowed to access the network, February 20, 2006 04:56:01
 */
...
#define LOG_ITEM_SIZE   128
	...
	if (strncasecmp(msg,"[Access Control]",strlen("[Access Control]")) == 0){
		cut_acl_log(msg,"with MAC address",log_max_len);
		return;
	}
	return;
}
