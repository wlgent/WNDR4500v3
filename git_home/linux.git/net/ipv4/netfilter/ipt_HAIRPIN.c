...
static unsigned int ipt_hairpin_target(struct sk_buff *skb, const struct xt_target_param *par)
{
	struct nf_conn *ct;
...
static bool ipt_hairpin_checkentry(const struct xt_tgchk_param *par)
{
	const struct nf_nat_multi_range_compat *mr = par->targinfo;
