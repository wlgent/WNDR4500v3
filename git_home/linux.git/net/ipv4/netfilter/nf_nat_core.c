...
static void nf_nat_cleanup_conntrack(struct nf_conn *ct)
{
	...
	if (!(nat->ct->status & IPS_PORT_FULL)) {
		unsigned int dport;
		...
		if (nat->ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum == IPPROTO_UDP) {
			if (nat->ct->status & IPS_SRC_NAT) {
				...
				if (0xC000 == (dport & 0xC000)) {
					if (dport & 0x1) {
						if (udp_tcp_port_flags & IP_NAT_UDP_ODD_PORT_FULL)
							udp_tcp_port_flags &= ~IP_NAT_UDP_ODD_PORT_FULL;
					} else {
						if (udp_tcp_port_flags & IP_NAT_UDP_EVEN_PORT_FULL)
							udp_tcp_port_flags &= ~IP_NAT_UDP_EVEN_PORT_FULL;
					}
				}
			}
			...
			if (nat->ct->status & IPS_SRC_NAT) {
				...
				if (0xC000 == (dport & 0xC000)) {
					if (dport & 0x1) {
						if (udp_tcp_port_flags & IP_NAT_TCP_ODD_PORT_FULL)
							udp_tcp_port_flags &= ~IP_NAT_TCP_ODD_PORT_FULL;
					} else {
						if (udp_tcp_port_flags & IP_NAT_TCP_EVEN_PORT_FULL)
							udp_tcp_port_flags &= ~IP_NAT_TCP_EVEN_PORT_FULL;
					}
				}
			}
