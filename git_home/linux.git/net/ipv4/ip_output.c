...
#ifdef CONFIG_IP_NF_IP_ALIGNMENT
	u32_set(&skb->priority, &sk->sk_priority);
	u32_set(&skb->mark, &sk->sk_mark);
#else
...
#ifdef CONFIG_IP_NF_IP_ALIGNMENT
			{
				u32_set(&fl.nl_u.ip4_u.saddr, &inet->saddr);
			}
#endif
...
#ifdef CONFIG_IP_NF_IP_ALIGNMENT
	u32_set(&ipc.addr, &rt->rt_src);
	u32_set(&daddr, &rt->rt_src);
#else
...
#ifdef CONFIG_IP_NF_IP_ALIGNMENT
		u32_set(&fl.nl_u.ip4_u.saddr, &rt->rt_spec_dst);
#endif
