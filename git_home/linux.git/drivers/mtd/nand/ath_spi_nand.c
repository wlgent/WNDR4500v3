...
ath_parse_read_id(ath_spi_nand_sc_t *sc)
{
	uint32_t id;

#define ATH_GIGA_SPI_NAND_V_ID			0xc8
#define ATH_GIGA_SPI_NAND_D_ID			0xf1
#define get_ath_spi_nand_vendor(x)		(((x) >> 8) & 0xff)
#define get_ath_spi_nand_device(x)		((x) & 0xff)

	ath_spi_nand_bit_banger(0x9F); //send cmd to spi nand
	...
	printk("%s: SPI NAND V.Id: 0x%x D.Id: 0x%x\n", __func__,
		get_ath_spi_nand_vendor(id), get_ath_spi_nand_device(id));

	if ((get_ath_spi_nand_vendor(id) != ATH_GIGA_SPI_NAND_V_ID) ||
	    (get_ath_spi_nand_device(id) != ATH_GIGA_SPI_NAND_D_ID))
		return 1;
...
{
#define ATH_GPIO_FN_SPI_CS1		0x25

	ath_gpio_set_fn(CONFIG_ATH_SPI_NAND_CS_GPIO, ATH_GPIO_FN_SPI_CS1);
...
static inline void
ath_spi_nand_cmd_read_from_cache(int start, int len, u_char *buf)
{
	int byte;
...
ath_spi_nand_page_read(struct mtd_info *mtd, loff_t off, u_char *buf, int len)
{
	...
	if (!buf) {
		...
		if (get_eccs(status) == ASNS_ECC_ERR_UNCORR) {
			eccfail = 1;
		}

		if (get_eccs(status) != ASNS_ECC_ERR_NONE) {
			ret = -EUCLEAN;
		}
	...
	ath_spi_nand_start();
	ath_spi_nand_cmd_read_from_cache(byte, len, buf);
	ath_spi_nand_end();
		...
		loff_t addr, size_t len, size_t *iodone)
{
	...
	*iodone = 0;
	...
	while (num) {
		...
		if (ret == -EUCLEAN){
			//euclean = 1;
			//printk("[0x%x] is corrected by ECC\n", (uint32_t)addr);
			euclean = 0;
		}
 ...
 */
static int __devinit ath_spi_nand_add_partition(ath_spi_nand_sc_t *sc)
{
	struct mtd_info *mtd = &sc->mtd;
...
static int __devexit ath_spi_nand_remove(void)
{
#ifdef CONFIG_MTD_PARTITIONS
	/* Deregister partitions */
	del_mtd_partitions(&ath_spi_nand_sc->mtd);
#endif
#if oob_support
	kfree(ath_spi_nand_sc->raw);
#endif
	kfree(ath_spi_nand_sc);
	ath_spi_nand_sc = NULL;
	return 0;
}
...
static struct nand_ecclayout ath_spi_nand_oob = {
	.eccbytes	= 16,
				...
				struct mtd_oob_ops *ops)
{
	...
	ath_spi_nand_start();
	ath_spi_nand_cmd_read_from_cache(byte, i, buf);
	ath_spi_nand_end();
	...
	if (ops->oobbuf) {
		...
		buf = sc->raw + mtd->writesize;
		for (oob = mtd->ecclayout->oobfree; oob->length; oob++) {
				...
				struct mtd_oob_ops *ops)
{
	...
	struct nand_oobfree *oobfree = mtd->ecclayout->oobfree;
			...
			struct mtd_oob_ops *ops)
{
	...
	/* EUCLEAN should not be returned if jffs2 read the oob area */
	if (ret == -EUCLEAN)
		return 0;
...
static int
ath_spi_nand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	int ret = -EIO, i, num, euclean = 0;
	struct mtd_oob_ops ath_ops = *ops;
	...
	if (ops->ooblen > mtd->oobavail) {
		/* request to read oob from multiple pages */
		ath_ops.ooblen = mtd->oobavail;
		num = ops->ooblen / mtd->oobavail;
		ath_ops.datbuf = NULL;
		ath_ops.len = 0;

		for (i = 0; i < num; i++) {
			ret = ath_spi_nand_rw_oob(mtd, 1 /* read */, from, &ath_ops);
			if (ret == -EUCLEAN)
				euclean = 1;
			...
			ops->oobretlen += ath_ops.oobretlen;
			ath_ops.oobbuf += mtd->oobavail;
		}
	} else {
...
{
	int ret, retlen;
	const int testblock = 0x80000;

	//ath_spi_nand_read_test(sc, mtd, testblock, 4096);

	ath_spi_nand_ecc(sc, 1);

	if (ath_spi_nand_block_erase(sc, testblock))
		printk("erase failed\n");
	...
	//ath_spi_nand_read_test(sc, mtd, testblock + 1024, 1024);	// u/a/s
	//ath_spi_nand_read_test(sc, mtd, testblock + 1000, 4096);	// u/a/m
	//ath_spi_nand_read_test(sc, mtd, testblock + 4096, 1000);	// a/u/s
	//ath_spi_nand_read_test(sc, mtd, testblock + 4096, 3000);	// a/u/m
	//ath_spi_nand_read_test(sc, mtd, testblock + 1000, 500);	// u/u/s
	//ath_spi_nand_read_test(sc, mtd, testblock + 1000, 3000);	// u/u/m
	ath_spi_nand_read_test(sc, mtd, testblock, 2048);
	ret = ath_spi_nand_write(mtd, testblock, 2048, &retlen, buf);
	if (ret == 0)
 ...
 */
static int __devinit ath_spi_nand_probe(void)
{
	ath_spi_nand_sc_t	*sc = NULL;
	struct mtd_info		*mtd = NULL;
	int			err = 0, bbt_size;

	if (!strstr(saved_command_line, DRV_NAME)) {
		printk("%s: No NAND partitions specified\n", __func__);
		return 0;
	}
	...
	if (sc->bbt) {
		...
		/*
		 * Per Yongqing Fu(GigaDevice/RDLV/BJ)'s e-mail
		 *	Block No. 768 is Factory Marked Bad
		 *	Any access to last block will be ignored
		 */
		ath_spi_nand_set_blk_state(mtd, 768 * mtd->erasesize,
							ATH_NAND_BLK_BAD);
		ath_spi_nand_set_blk_state(mtd, mtd->size - mtd->erasesize,
							ATH_NAND_BLK_BAD);
	}
	ath_spi_nand_sc = sc;
...
static int __init ath_spi_nand_init(void)
{
	int ret;
...
static void __exit ath_spi_nand_exit(void)
{
	ath_spi_nand_remove();
}

module_init(ath_spi_nand_init);
module_exit(ath_spi_nand_exit);
