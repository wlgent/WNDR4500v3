...
#define ath_nor_flash_bit_banger(__b)	\
	ath_spi_bit_banger(ATH_SPI_CE_LOW, ATH_SPI_CE_HIGH, __b)
#define ath_nor_flash_go()	ath_spi_go(ATH_SPI_CE_LOW)
#define ath_nor_flash_send_addr(__a)	\
	ath_spi_send_addr(ATH_SPI_CE_LOW, ATH_SPI_CE_HIGH, __a)
/*
 * bank geometry
 */
...
ath_spi_flash_unblock(void)
{
	...
	ath_nor_flash_bit_banger(ATH_SPI_CMD_WRITE_SR);
	ath_nor_flash_bit_banger(0x0);
	ath_nor_flash_go();
	ath_spi_poll();
}
#endif
		   ...
		   size_t * retlen, const u_char * src)
{
	...
	*retlen = len;
	...
	for (; len; len--, dst++, src++) {
		...
		ath_nor_flash_bit_banger(ATH_SPI_CMD_PAGE_PROG);
		ath_nor_flash_send_addr(dst);

		val = *src & 0xff;
		ath_nor_flash_bit_banger(val);
...
ath_spi_write_enable()
{
	...
	ath_nor_flash_bit_banger(ATH_SPI_CMD_WREN);
	ath_nor_flash_go();
}
...
ath_spi_poll()
{
	...
	do {
		ath_reg_wr_nf(ATH_SPI_WRITE, ATH_SPI_CS_DIS);
		ath_nor_flash_bit_banger(ATH_SPI_CMD_RD_STATUS);
		ath_spi_delay_8(ATH_SPI_CE_LOW, ATH_SPI_CE_HIGH);
...
ath_spi_write_page(uint32_t addr, uint8_t *data, int len)
{
	...
	ath_spi_write_enable();
	ath_nor_flash_bit_banger(ATH_SPI_CMD_PAGE_PROG);
	ath_nor_flash_send_addr(addr);

	for (i = 0; i < len; i++) {
		ch = *(data + i);
		ath_nor_flash_bit_banger(ch);
	}
...
{
	ath_spi_write_enable();
	ath_nor_flash_bit_banger(ATH_SPI_CMD_SECTOR_ERASE);
	ath_nor_flash_send_addr(addr);
	ath_nor_flash_go();
#if 0
	/*
