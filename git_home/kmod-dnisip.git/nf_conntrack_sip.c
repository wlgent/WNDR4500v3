...
static int process_via(struct sk_buff *skb,
		       const char **dptr, unsigned int *datalen)
{
	enum ip_conntrack_info ctinfo;
	struct nf_conn *ct = nf_ct_get(skb, &ctinfo);
	...
	if ((ct_sip_parse_header_uri(ct, *dptr, NULL, *datalen,
	     SIP_HDR_VIA, NULL, &matchoff, &matchlen, &daddr, &port) > 0) &&
		...
		if (nf_nat_sip_expect && ct->status & IPS_NAT_MASK)
			ret = nf_nat_sip_expect(skb, dptr, datalen, exp, matchoff, matchlen);
		else {
...
static int process_via_sdp(struct sk_buff *skb,
		       const char **dptr, unsigned int *datalen,
		       unsigned int cseq)
{
	if (process_via(skb, dptr, datalen) == NF_DROP)
		return NF_DROP;
...
static const struct sip_handler sip_handlers[] = {
	SIP_HANDLER("INVITE", process_via_sdp, process_invite_response),
