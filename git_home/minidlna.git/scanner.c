...
ScanDirectory(const char *dir, const char *parent, media_types dir_types)
{
	...
	for (i=0; i < n; i++)
	{
        ...
        start_retry:
                         if (SQLITE_OK != sqlite3_exec(db, "begin transaction", 0, 0, &errMsg)) {
                                 DPRINTF(E_WARN, L_SCANNER,"Begin transaction Err :%s ...\n",errMsg);
                                sleep(5);
                                goto start_retry;
                         } else {
                                 DPRINTF(E_WARN, L_SCANNER,"Begin transaction OK ...\n");
                         }
        ...
        commit_retry:
                         if (SQLITE_OK != sqlite3_exec(db, "commit transaction", 0, 0, &errMsg)) {
                                 DPRINTF(E_WARN, L_SCANNER,"Commit transaction Err :%s ...\n",errMsg);
                                sleep(5);
                                goto commit_retry;
 
                         } else {
                                 DPRINTF(E_WARN, L_SCANNER,"Commit transaction OK ...\n");
                         }
