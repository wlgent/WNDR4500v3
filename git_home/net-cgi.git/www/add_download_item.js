...
function get_ids(cf,method)
{
	...
	for( i=0; i< objs.length; i++ )
	{
		if( objs[i].checked == true )
		{
			...
			{
				if( stat != "$greendl_common_showStatusPaused" && stat != "$wps_add_failure")
				{
					resume_count++;
					continue;
				}
...
function check_magnet(magnet_url)
{
	...
	{
		var index = magnet_url.toUpperCase().lastIndexOf(".TORRENT");
		if(index != -1)
		{
			magnet_url = magnet_url.substr(0,index+8);
		}
...
function refresh_download_table()
{
	...
	var dl_tb_html,chbx_all_html;
	var wform = "document.forms[0]";
	if ( this_download_type == "bt" )
		wform = "document.forms[2]";

	if(disk_num == 0 || (disk_num !=0 && green_download_enable == "0"))
		chbx_all_html = "<TD nowrap align=center><span class=\"subhead\">$greendl_common_showSelect<input type=checkbox name=\"chbx_all_item\" disabled></span></TD>"
	else
		chbx_all_html = "<TD nowrap align=center><span class=\"subhead\">$greendl_common_showSelect<input type=checkbox name=\"chbx_all_item\" value=\"0\" onclick=\"select_all_items()\"></span></TD>";
