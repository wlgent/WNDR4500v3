function gray_rule_table()
{
	...
	if(cf.qosEnable.checked == true && enable_ap_flag != "1")
	{
...
// compare_type: 2: applications name or on-line game name, 8: mac address
	...
	cf = document.forms[0];
	for( var x =1; x <=l_num; x=(new_qos_flag == 1) ? x+2 : x+1) // edit a exist item
	{
		...
		{// find a exist item
			...
			if(new_qos_flag == 1) // new qos, every rule have 2 configs
			{
				num++;
...
function check_prru_qos_apply(cf)
{
	...
	qos_bandwith=parseFloat(cf.tcbw_value.value);
	if(isNaN(qos_bandwith))
	{
		...
		if (max_bandwidth == 1000)
		{
			if(cf.tcbw_unit.selectedIndex == 0)
			{
				if(!(qos_bandwith > 0 && qos_bandwith < 1024001))
				{
					...
					cf.qos_hidden_uprate.value = parseFloat(cf.tcbw_value.value);
			}
			else
			{
				if(!(qos_bandwith > 0 && qos_bandwith < 1001))
				{
					...
					cf.qos_hidden_uprate.value = parseFloat(cf.tcbw_value.value)*1024;
			}
		}
		else if (max_bandwidth == 100)
		{
	...
	{
		cf.qos_hidden_uprate.value=parseFloat(tcbw_value);
...
function check_qos_add_info()
{
	...
	if(check_name_space(cf.ser_poli_name.value)==false)
		return false;
	if((parent.is_pr_version == 1 && index == 40) || (parent.is_ru_version == 1 && index == 33)|| (parent.is_ru_version != 1 && parent.is_pr_version != 1 && index == 31))
	{
...
function check_ookla_speedtest(form)
{
	...
	form.action="/func.cgi?/QOS_prru_main.htm timestamp="+ts;
