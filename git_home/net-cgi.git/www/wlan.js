	...
	for ( i = 0; i < tmp_array.length; i++ )
	{
		if ( tmp_array[i].indexOf("(DFS)") > -1 )
		{
			if ( 0 == hidden_dfs_channel && ( 1 == dfs_channel_router_flag ||
				...
				( dfs_channel3_router_flag == 1 && index == 6 ) ) ) //Japan, United States
			{
...
*/
	if(!( get_endis_guestNet == 1 && endis_wl_radio == 1) && !( get_endis_guestNet_an == 1 && endis_wla_radio_an == 1) && cf.enable_bssid.checked == false && cf.enable_bssid_an.checked == false)//no enable
		cf.hidden_change_guest.value="0";
	else
		cf.hidden_change_guest.value="1";
