...
function check_pppoe(form,check)
{
	...
	{
		var reboot_flag=0;
		if((form.MACAssign[0].checked && pppoe_get_mac_assign!="0") || (form.MACAssign[1].checked && pppoe_get_mac_assign!="1") || (form.MACAssign[2].checked && pppoe_get_mac_assign!="2") )
			reboot_flag=1;
		else if(form.MACAssign[2].checked && pppoe_get_this_mac!= form.Spoofmac.value)
			reboot_flag=1;
		if(reboot_flag == 1)
		{
			if(confirm("$mac_change_warn") == false)
				return false;
			form.hid_hnat_reboot.value = "1";
		}
	}
