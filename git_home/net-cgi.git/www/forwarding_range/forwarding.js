...
function Check_add(cf)     
{
	...
	for(i=1;i<=trigger_array_num;i++)
	{
		...
		if (!(parseInt(conend_port)<parseInt(input_external_start_port)||parseInt(input_external_end_port)<parseInt(constart_port) ) )
		{
	...
	for(i=1;i<=upnp_array_num;i++)
	{
		...
		upnp_ip=each_info[4];
		if (!( parseInt(upnp_ext)<parseInt(input_external_start_port) || parseInt(input_external_end_port)<parseInt(upnp_ext) ) && input_ip != upnp_ip)
		{
			alert(invalid_port_used);
			return false;
		}
	...
	{
		if(check_readyshare_port(input_external_start_port, input_external_end_port, "WAN") == false)
		{
			alert(invalid_port_used);
			return false;
		}
	...
	//check with remote
	if (!(parseInt(remote_port)<parseInt(input_external_start_port)||parseInt(input_external_end_port)<parseInt(remote_port)) && remote_port!="")
	{
		alert(invalid_port_used);
		return false;
	}		
...
function check_forwarding_add_range(cf,flag)
{
	...
	var service_name=cf.portname.value.replace(/ /g, "&harr;");
	for(i=1;i<=forward_array_num;i++)
	{
		var str = eval ( 'forwardingArray' + i ).replace(/&#92;/g, "\\").replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&#40;/g,"(").replace(/&#41;/g,")").replace(/&#34;/g,'\"').replace(/&#39;/g,"'").replace(/&#35;/g,"#").replace(/&#38;/g,"&");
		var each_info=str.split(' ');
		...
		if(flag == 'edit')
		{
			if(each_info[0]==service_name && !(select_editnum<=i && i<parseInt(select_editnum)+parseInt(edit_num)) && each_info[7]==0)
			{
				alert("$forwarding_ser_name_dup");
				return false;
			}
	...
	for(k=0;k<input_ext_start_port.length-1;k++)
	{
		...
		for(i=1;i<=trigger_array_num;i++)
		{
			...
			constart_port=each_info[6];	
			conend_port=each_info[7];	
			if (!(parseInt(conend_port)<parseInt(input_ext_start_port[k])||parseInt(input_ext_end_port[k])<parseInt(constart_port) ))
			{
				if (  (parseInt(conend_port)<parseInt(input_int_start_port[k])||parseInt(input_int_end_port[k])<parseInt(constart_port) ) ||
					( (consertype == "TCP/UDP" || input_sertype == "TCP/UDP" || input_sertype == consertype) && (triggeringip == "any" || input_ip != triggeringip )) )
				{
					alert(invalid_port_used);
					return false;
				}
		...
		for(i=1;i<=upnp_array_num;i++)
		{
			...
			upnp_ext=each_info[3];
			upnp_ip=each_info[4];
			if (!( parseInt(upnp_ext)<parseInt(input_ext_start_port[k]) || parseInt(input_ext_end_port[k])<parseInt(upnp_ext) ) )
			{
				if ( ( parseInt(upnp_ext)<parseInt(input_int_start_port[k]) || parseInt(input_int_end_port[k])<parseInt(upnp_int) ) ||
					( (upnp_protocal == "TCP/UDP" || input_sertype == "TCP/UDP" || input_sertype == upnp_protocal) && input_ip != upnp_ip ) )
				{
					alert(invalid_port_used);
					return false;
				}
		...
		if( usb_router_flag == 1)
		{
			...
			{
				if(check_readyshare_port(input_ext_start_port[k], input_ext_end_port[k], "WAN") == false)
				{
					alert(invalid_port_used);
					return false;
				}
		...
		{
			if (!(parseInt(remote_port)<parseInt(input_ext_start_port[k])||parseInt(input_ext_end_port[k])<parseInt(remote_port)) && remote_port!="")
			{
				alert(invalid_port_used);
				return false;
			}		
		...
		if( parseInt(input_ext_start_port[k])<=123 && parseInt(input_ext_end_port[k])>=123 && endis_ntp == "1" && cf.srvtype.value != "TCP")
		{
			alert(invalid_port_used);
			return false;
		}
		...
		if(parseInt(input_ext_start_port[k])<=1900 && parseInt(input_ext_end_port[k])>=1900 && endis_upnp == "1" && cf.srvtype.value != "TCP")
		{
			alert(invalid_port_used);
			return false;
		}
		...
		if(parseInt(input_ext_start_port[k])<=5050 && parseInt(input_ext_end_port[k])>=5050 && info_get_wanproto == "bigpond" && cf.srvtype.value != "TCP")
		{
			alert(invalid_port_used);
			return false;
		}
	...
	cf.portname.value=service_name;
