function initPage()
{
	if ( top.cd_less_download == 0 || master != "admin")
		return;
	
	top.download_genie = 1;
	top.download_vault = 1;
...
function app_choose(id)
{
	...
	if ( id == "genie_choose" )
		top.download_genie = flag;
	else
		top.download_vault = flag;
...
function toDownload()
{
	...
	if ( !top.download_genie && !top.download_vault )
	{
		return toFwChecking();
	}
	else
	{
		...
		setTimeout(function(){
			...
			else if ( !top.download_genie && top.download_vault )
			{
			...
			if ( top.download_vault )
				document.body.appendChild(ifr2);
			if ( top.download_genie )
