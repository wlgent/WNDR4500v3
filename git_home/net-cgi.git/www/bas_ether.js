...
function check_ether(cf,check)
{
	...
	{
		var reboot_flag=0;
		if((cf.MACAssign[0].checked && ether_get_mac_assign!="0") || (cf.MACAssign[1].checked && ether_get_mac_assign!="1") || (cf.MACAssign[2].checked && ether_get_mac_assign!="2") )
			reboot_flag=1;
		else if(cf.MACAssign[2].checked && ether_get_this_mac!= cf.Spoofmac.value)
			reboot_flag=1;
		if(reboot_flag == 1)
		{
			if(confirm("$mac_change_warn") == false)
				return false;
			cf.hid_hnat_reboot.value = "1";
		}
	}
