function check_reservation_add(cf,flag)
{
	...
	for(i=1;i<=array_num;i++)
	{
		...
		if(flag == 'edit')
		{
			if(select_editnum!=i)
			{
				if( (is_ipmac == 0 && ((each_info[2]==cf.dv_name.value && each_info[2]!="<unknown>") || rsvipaddr == each_info[0] || cf.rsv_mac.value.toLowerCase() == each_info[1].toLowerCase())) || (is_ipmac == 1 && (rsvipaddr == each_info[0] || cf.rsv_mac.value.toLowerCase() == each_info[1].toLowerCase())) )//bug40808
				{//44938
...
function data_select(num)
{
	...
	if( show_ip_array[num] != "----" && show_ip_array[num] != "<unknown>" &&  show_ip_array[num] != "&lt;unknown&gt;" && show_ip_array[num] != "&lt;unknown&gt" )
	{
